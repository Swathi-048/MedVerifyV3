...
def ocr_text(img):
    ...
    try:
        ...
        t1 = pytesseract.image_to_string(pil, config="--psm 6")
        t2 = pytesseract.image_to_string(pil, config="--psm 11")
        return (t1 + " " + t2).upper().strip()
    except:
        return ""
    ...
    if ref_name:
        ...
        if ref in text:
            return result("Drug Name Spelling", "PASS", 1.0,
                          f"'{ref_name}' found correctly spelled on packaging")

        # Check substitutions
        fake_ver = ref
        for fake, real in subs.items():
            fake_ver = fake_ver.replace(real, fake)
        if fake_ver in text and fake_ver != ref:
            return result("Drug Name Spelling", "FAIL", 0.0,
                          f"Letter substitution detected — '{fake_ver}' found instead of '{ref}'")

        # Check edit distance
        words = re.findall(r"[A-Z0-9]{3,}", text)
        ref_words = re.findall(r"[A-Z0-9]{3,}", ref)
        ...
        return result("Drug Name Spelling", "FAIL", 0.0,
                      f"Drug name '{ref_name}' not found on packaging")
    else:
        # No reference — just check if any readable drug-like word exists
...
def check_brand_name(text, ref_brand=None):
    ...
    if ref_brand:
        ...
        min_dist = min((levenshtein(w, brand) for w in words), default=99)
        if min_dist <= 3:
            return result("Brand Name", "WARNING", 0.5,
                          f"Brand name may be misspelled (edit distance: {min_dist})")
        return result("Brand Name", "FAIL", 0.0,
                      f"Brand '{ref_brand}' not found on packaging")
    else:
        # Look for known pharma keywords
        pharma_keywords = ["LIMITED","LTD","PHARMA","LABS","INDUSTRIES",
                           "PHARMACEUTICALS","PVT","INC","CORP"]
...
def check_packaging_color(img, ref_img=None):
    ...
    def dominant_color(i):
        ...
        return int(np.argmax(h_hist)) * 10  # dominant hue in degrees
    ...
    if ref_img is not None:
        ...
        if s_std < 60:
            return result("Packaging Color", "PASS", 1.0,
                          "Packaging color appears consistent and uniform")
        else:
            return result("Packaging Color", "WARNING", 0.5,
                          "Packaging color variation detected — compare with genuine pack")
...
def check_layout(img, ref_img=None):
    ...
    if ref_img is None:
        return result("Layout & Design", "WARNING", 0.5,
                      "No reference image available for layout comparison")
    try:
...
def check_texture(img, ref_img=None):
    ...
    if ref_img is not None:
        ...
        # Standalone — check texture consistency
        gray = to_gray(resize(img, (200,200)))
        lbp = local_binary_pattern(gray, P=8, R=1, method="uniform")
        uniformity = float(np.std(lbp))
        if uniformity < 3.0:
            return result("Packaging Texture", "PASS", 1.0,
                          "Packaging texture appears consistent")
        else:
            return result("Packaging Texture", "WARNING", 0.5,
                          "Packaging texture is inconsistent — inspect carefully")
...
def check_visual_similarity(img, ref_img=None):
    ...
    if ref_img is None:
        return result("Visual Similarity", "WARNING", 0.5,
                      "No reference image in database for this medicine")
    g1 = resize(to_gray(img))
    g2 = resize(to_gray(ref_img))
...
def check_batch_number(text):
    ...
    if re.search(r"\b[A-Z]{1,4}\d{4,10}\b", text):
        ...
        return result("Batch Number", "PASS", 1.0,
                      f"Batch number found: {match}")
    # Also check for "BATCH" keyword followed by anything
    if re.search(r"BATCH[\s\w]{2,15}", text):
        return result("Batch Number", "PASS", 1.0,
                      "Batch number field detected on packaging")
...
def check_mfg_date(text):
    ...
    patterns = [
        r"(?:MFD|MFG|MANUFACTURED|MANUF)[:\s.]*(\d{2})[/\-.](\d{4})",
        r"(?:MFD|MFG)[:\s.]*(\d{2})[/\-.](\d{2})",
        r"(?:MFD|MFG)[:\s.]*([A-Z]{3,})[/\-.\s](\d{4})",
    ]
    for pat in patterns:
        if re.search(pat, text):
            m = re.search(pat, text).group()
            return result("Manufacturing Date", "PASS", 1.0,
                          f"Manufacturing date found: {m}")
    if "MFD" in text or "MFG" in text or "MANUFACTURED" in text:
        return result("Manufacturing Date", "WARNING", 0.5,
                      "MFD keyword found but date format unclear")
...
def check_expiry_date(text):
    ...
    patterns = [
        r"(?:EXP|EXPIRY|EXPDATE|USE\s*BEFORE)[:\s.]*(\d{2})[/\-.](\d{4})",
    ...
    for pat in patterns:
        ...
        if m:
            try:
                ...
                pass
    if "EXP" in text or "EXPIRY" in text:
        return result("Expiry Date", "WARNING", 0.5,
                      "Expiry date keyword found but date unreadable — possible tampering")
    return result("Expiry Date", "FAIL", 0.0,
    ...
    for pat in patterns:
        ...
        if m:
            return result("License Number", "PASS", 1.0,
                          f"Manufacturing license found: {m.group()[:30]}")
